...
static int allreduce_best(lua_State *L) {
  // get args
  THFloatTensor *data = (THFloatTensor *)luaT_checkudata(L, 1, torch_FloatTensor_id);
  const char *master_location_c = lua_tostring(L, 2);
  std::string master_location(master_location_c);
  ...
  // array of scores
  float *scores = (float *)calloc(sizeof(float), total);
  scores[node] = score;
  all_reduce((char*)scores, sizeof(float)*total, master_location, unique_id, total, node);
  int max = 0;
  int idx, i;
  for (i=0; i<total; i++) {
    if (scores[i] >= max) {
  ...
  // all reduce
  if (idx != node) THFloatTensor_zero(data);
  struct timeb t_start, t_end;
  double net_comm_time = 0.;
  ftime(&t_start);
  all_reduce((char*)raw, n, master_location, unique_id, total, node);
